...
describe('date_utils', () => {
    ...
    describe('stringToDate', () => {
        it('should parse an ISO date-only string to a Date', () => {
            const result = stringToDate('2025-01-15');
            expect(result).toBeInstanceOf(Date);
            expect(result?.toISOString().slice(0, 10)).toBe('2025-01-15');
        });

        it('should resolve relative dates before parsing', () => {
            const today = stringToDate(DateReference.TODAY);
            expect(today?.toISOString().slice(0, 10)).toBe('2025-01-15');

            const tomorrow = stringToDate(DateReference.TOMORROW);
            expect(tomorrow?.toISOString().slice(0, 10)).toBe('2025-01-16');
        });

        it('should resolve dynamic relative patterns before parsing', () => {
            const result = stringToDate('+7d');
    ...
    describe('dateToString', () => {
        it('should format a Date to YYYY-MM-DD', () => {
            ...
            expect(dateToString(date)).toBe('2025-03-05');
        });
        ...
        it('should return null for an invalid Date', () => {
            expect(dateToString(new Date('invalid'))).toBeNull();
        });
    });

    describe('stringToDate / dateToString round trip', () => {
        it('should round-trip a date-only value unchanged', () => {
            const original = '2025-07-04';
            const date = stringToDate(original);
            expect(date).not.toBeNull();
            expect(dateToString(date)).toBe(original);
        });
    });
});
