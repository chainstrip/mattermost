...
import {getFileType, getLocaleDateFromUTC, getNextBillingDate, getSuggestionBoxAlgn, makeIsEligibleForClick} from './utils';
...
describe('Utils.getLocaleDateFromUTC', () => {
    beforeEach(() => {
        jest.useFakeTimers();
    ...
    test('formats a unix timestamp with the default format', () => {
        // 2024-06-15T12:34:56Z
        const result = getLocaleDateFromUTC(1718454896);
        expect(result).toBe('2024/06/15 12:34:56');
    });

    test('formats a unix timestamp with a custom format', () => {
        const result = getLocaleDateFromUTC(1718454896, 'YYYY-MM-DD');
        expect(result).toBe('2024-06-15');
    });

    test('appends the timezone abbreviation when a userTimezone is given', () => {
        const result = getLocaleDateFromUTC(1718454896, 'YYYY-MM-DD', 'America/New_York');
        expect(result).toBe('2024-06-15 EST');
    });

    test('returns the current time (unformatted) when timestamp is falsy', () => {
        const result = getLocaleDateFromUTC(0);
        expect(result).toBe(Date.now());
    });
});

describe('Utils.getNextBillingDate', () => {
    afterEach(() => {
        jest.useRealTimers();
    });

    test('returns the first day of next month formatted as "MMM D, YYYY"', () => {
        jest.useFakeTimers();
        jest.setSystemTime(new Date('2025-01-15T10:00:00.000Z'));

        expect(getNextBillingDate()).toBe('Feb 1, 2025');
    });

    test('rolls over into next year when the current month is December', () => {
        jest.useFakeTimers();
        jest.setSystemTime(new Date('2025-12-20T10:00:00.000Z'));

        expect(getNextBillingDate()).toBe('Jan 1, 2026');
    });
});
