...
import type {PluginsState, PostDropdownMenuAction} from 'types/store/plugins';
...
describe('components — PostDropdownMenu nested submenu registration', () => {
    function menuAction(overrides: Partial<PostDropdownMenuAction> & {id: string; pluginId: string}): PostDropdownMenuAction {
        return {
            text: overrides.id,
            action: () => {},
            filter: () => true,
    ...
    it('registers a root menu item with no parentMenuId as a plain entry', () => {
        const state = pluginReducers(undefined, {
            type: ActionTypes.RECEIVED_PLUGIN_COMPONENT,
            name: 'PostDropdownMenu',
            data: menuAction({id: 'root-menu', pluginId: 'plugin-a'}),
        });

        expect(state.components.PostDropdownMenu).toHaveLength(1);
        expect(state.components.PostDropdownMenu[0]).toMatchObject({id: 'root-menu', pluginId: 'plugin-a'});
        expect(state.components.PostDropdownMenu[0].subMenu).toBeUndefined();
    });

    // NOTE (found while writing this test, not fixed here per harness rules):
    // hasMenuId() returns false whenever the candidate menu's own `subMenu` is
    // falsy, BEFORE it ever compares ids - so a freshly registered root menu
    // (subMenu undefined) can never be found as a nesting target. remove()
    // therefore always removes zero elements for this, the most common case
    // (attaching a menu's first child), menu[0] is undefined, and the reducer
    // takes the `return state;` early-out: the child registration is silently
    // dropped. This test pins that actual (buggy) behaviour.
    it('silently drops a submenu registration when the parent has no existing subMenu array (bug: hasMenuId short-circuits on falsy subMenu before checking id)', () => {
        let state = pluginReducers(undefined, {
            type: ActionTypes.RECEIVED_PLUGIN_COMPONENT,
            name: 'PostDropdownMenu',
            data: menuAction({id: 'root-menu', pluginId: 'plugin-a'}),
        });
        const beforeSecondDispatch = state;

        state = pluginReducers(state, {
            type: ActionTypes.RECEIVED_PLUGIN_COMPONENT,
            name: 'PostDropdownMenu',
            data: menuAction({id: 'child-menu', pluginId: 'plugin-a', parentMenuId: 'root-menu'}),
        });

        // Reducer hit the `return state;` early-out: same reference, root unchanged.
        expect(state).toBe(beforeSecondDispatch);
        expect(state.components.PostDropdownMenu).toHaveLength(1);
        expect(state.components.PostDropdownMenu[0].id).toBe('root-menu');
        expect(state.components.PostDropdownMenu[0].subMenu).toBeUndefined();
    });

    it('removes and rebuilds the parent entry when it already carries a subMenu array', () => {
        // Seeds a parent that already has a (empty) subMenu array, which is the one
        // shape hasMenuId's guard lets through. This drives the real remove()/
        // buildMenu() nesting path end to end: the parent is located, spliced out
        // of the top-level array by lodash's remove(), rebuilt with the new child
        // attached, and pushed back.
        const seededState = pluginReducers(undefined, {type: '@@INIT'} as any);
        const withParent = {
            ...seededState,
            components: {
                ...seededState.components,
                PostDropdownMenu: [menuAction({id: 'root-menu', pluginId: 'plugin-a', subMenu: []})],
            },
        };

        const state = pluginReducers(withParent, {
            type: ActionTypes.RECEIVED_PLUGIN_COMPONENT,
            name: 'PostDropdownMenu',
            data: menuAction({id: 'child-menu', pluginId: 'plugin-a', parentMenuId: 'root-menu'}),
        });
        ...
        expect(state.components.PostDropdownMenu).toHaveLength(1);
        const root = state.components.PostDropdownMenu[0];
        expect(root.id).toBe('root-menu');
        expect(root.subMenu).toHaveLength(1);
        expect(root.subMenu?.[0].id).toBe('child-menu');
    });

    it('leaves state untouched when parentMenuId does not match any registered menu', () => {
        let state = pluginReducers(undefined, {
            type: ActionTypes.RECEIVED_PLUGIN_COMPONENT,
            name: 'PostDropdownMenu',
            data: menuAction({id: 'root-menu', pluginId: 'plugin-a'}),
        ...
        state = pluginReducers(state, {
            type: ActionTypes.RECEIVED_PLUGIN_COMPONENT,
            name: 'PostDropdownMenu',
            data: menuAction({id: 'orphan-menu', pluginId: 'plugin-a', parentMenuId: 'does-not-exist'}),
        });
    ...
    it('only removes and re-parents the entry belonging to the same plugin', () => {
        let state = pluginReducers(undefined, {
            type: ActionTypes.RECEIVED_PLUGIN_COMPONENT,
            name: 'PostDropdownMenu',
            data: menuAction({id: 'shared-id', pluginId: 'plugin-a'}),
        });

        // A different plugin's menu with the parent's id should not match (pluginId check),
        // so this is treated as an unresolved parent and state is returned unchanged.
        const beforeDispatch = state;
        state = pluginReducers(state, {
            type: ActionTypes.RECEIVED_PLUGIN_COMPONENT,
            name: 'PostDropdownMenu',
            data: menuAction({id: 'child-menu', pluginId: 'plugin-b', parentMenuId: 'shared-id'}),
        });

        expect(state).toBe(beforeDispatch);
    });
});
