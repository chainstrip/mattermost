...
describe('components/ColorInput', () => {
    ...
    test('should revert to the previous valid value on blur when the typed text is not a valid color', async () => {
        ...
        await userEvent.clear(inputElement);
        await userEvent.type(inputElement, 'zzzzzz');

        expect(inputElement).toHaveValue('zzzzzz');
        ...
        // tinycolor().isValid() is false for 'not-a-color', so onBlur falls back
        // to the last value received from props instead of committing garbage.
        expect(inputElement).toHaveValue('#336699');
