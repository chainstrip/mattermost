...
describe('integration utils', () => {
    ...
    describe('relative min_date/max_date bounds (resolveBoundToDate)', () => {
        // resolveBoundToDate is only reachable through checkDialogElementForError's
        // range validation, so pin the clock and drive it from there.
        beforeEach(() => {
            jest.useFakeTimers();
        ...
        it('resolves "today" as a min_date bound', () => {
            const element = TestHelper.getDialogElementMock({
                type: 'datetime',
                datetime_config: {min_date: 'today'},
            });

            // Before the start of today (in UTC) should fail.
            expect(checkDialogElementForError(element, '2025-06-14T23:00:00Z')?.id).toBe('interactive_dialog.error.before_min_date');

            // Any time today or later should pass.
            expect(checkDialogElementForError(element, '2025-06-15T00:00:00Z')).toBeNull();
        });

        it('resolves "tomorrow" as a max_date bound', () => {
            const element = TestHelper.getDialogElementMock({
                type: 'datetime',
                datetime_config: {max_date: 'tomorrow'},
            });

            // End of tomorrow's day is beyond the "tomorrow" (start-of-day) bound.
            expect(checkDialogElementForError(element, '2025-06-16T23:00:00Z')?.id).toBe('interactive_dialog.error.after_max_date');
            expect(checkDialogElementForError(element, '2025-06-16T00:00:00Z')).toBeNull();
        });

        it('resolves "yesterday" as a min_date bound', () => {
            const element = TestHelper.getDialogElementMock({
                type: 'datetime',
                datetime_config: {min_date: 'yesterday'},
            });

            expect(checkDialogElementForError(element, '2025-06-13T12:00:00Z')?.id).toBe('interactive_dialog.error.before_min_date');
            expect(checkDialogElementForError(element, '2025-06-14T01:00:00Z')).toBeNull();
        });

        it('resolves dynamic day/week/month offsets (+Nd, +Nw, +Nm)', () => {
            const dayBound = TestHelper.getDialogElementMock({
                type: 'datetime',
                datetime_config: {min_date: '+2d'},
            });
            expect(checkDialogElementForError(dayBound, '2025-06-16T12:00:00Z')?.id).toBe('interactive_dialog.error.before_min_date');
            expect(checkDialogElementForError(dayBound, '2025-06-17T12:00:00Z')).toBeNull();

            const weekBound = TestHelper.getDialogElementMock({
                type: 'datetime',
                datetime_config: {max_date: '+1w'},
            });
            expect(checkDialogElementForError(weekBound, '2025-06-23T12:00:00Z')?.id).toBe('interactive_dialog.error.after_max_date');
            expect(checkDialogElementForError(weekBound, '2025-06-21T12:00:00Z')).toBeNull();

            const monthBound = TestHelper.getDialogElementMock({
                type: 'datetime',
                datetime_config: {max_date: '+1m'},
            });
            expect(checkDialogElementForError(monthBound, '2025-07-16T12:00:00Z')?.id).toBe('interactive_dialog.error.after_max_date');
            expect(checkDialogElementForError(monthBound, '2025-07-10T12:00:00Z')).toBeNull();
        });

        it('resolves dynamic hour/minute/second offsets (+NH, +NM, +NS)', () => {
            const hourBound = TestHelper.getDialogElementMock({
                type: 'datetime',
                datetime_config: {min_date: '+2H'},
            });
            // now = 2025-06-15T12:00:00Z, +2H = 14:00:00Z
            expect(checkDialogElementForError(hourBound, '2025-06-15T13:00:00Z')?.id).toBe('interactive_dialog.error.before_min_date');
            expect(checkDialogElementForError(hourBound, '2025-06-15T15:00:00Z')).toBeNull();

            const minuteBound = TestHelper.getDialogElementMock({
                type: 'datetime',
                datetime_config: {max_date: '+30M'},
            });
            expect(checkDialogElementForError(minuteBound, '2025-06-15T12:45:00Z')?.id).toBe('interactive_dialog.error.after_max_date');
            expect(checkDialogElementForError(minuteBound, '2025-06-15T12:15:00Z')).toBeNull();
        });

        it('resolves a negative dynamic offset (-Nd) for a min_date bound', () => {
            const element = TestHelper.getDialogElementMock({
                type: 'datetime',
                datetime_config: {min_date: '-3d'},
            });

            expect(checkDialogElementForError(element, '2025-06-11T12:00:00Z')?.id).toBe('interactive_dialog.error.before_min_date');
            expect(checkDialogElementForError(element, '2025-06-13T12:00:00Z')).toBeNull();
        });
    });
});
